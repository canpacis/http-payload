...
func (e *Encoder) Encode(v any) error {
	...
	for i := range rv.NumField() {
		...
		tag, ok := field.Tag.Lookup(e.key)
		if !ok {
			continue
		}

		f, ok := e.setter.(interface {
			SetField(string, any, reflect.StructField)
		})
		if ok {
			f.SetField(tag, value.Interface(), field)
		} else {
			e.setter.Set(tag, value.Interface())
		}
	}
...
func NewDecoder(getter Getter, key string) *Decoder {
	return &Decoder{
		getter: getter,
		key:    key,
	}
...
func NewEncoder(setter Setter, key string) *Encoder {
	return &Encoder{
		setter: setter,
		key:    key,
	}
